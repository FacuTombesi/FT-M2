...
export default function Bienvenido () {
  return (
    <div className={styles.divBienvenido}>
      <h1 className={styles.title} style={{color: 'red'}}>Skills</h1>
      <h3 className={styles.subtitle}>{studentName}</h3>
      <ul className={styles.unorderedList}>
        {techSkills.map(skill => (
          <li className={styles.listItem} key={skill}>{skill.tech}<img className={styles.listItem} src={skill.image} alt={skill.tech} /></li>
        ))}
      </ul>
